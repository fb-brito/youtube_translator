...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.last_original_file_path = None
        self.last_translated_file_path = None
        ...
        self.transcript_button = ctk.CTkButton(self.control_frame, text="1. Baixar Transcrição", command=lambda: self.start_task(download_transcript))
        self.transcript_button.grid(row=1, column=0, padx=5, pady=5, sticky="ew")

        self.transcribe_audio_button = ctk.CTkButton(self.control_frame, text="1. Transcrever via Áudio", command=lambda: self.start_task(transcribe_audio_local))
        self.transcribe_audio_button.grid(row=1, column=1, padx=5, pady=5, sticky="ew")

        self.translate_button = ctk.CTkButton(self.control_frame, text="2. Traduzir para PT-BR", command=self.start_translate_task, state="disabled")
        self.translate_button.grid(row=2, column=0, padx=5, pady=5, sticky="ew")

        self.tts_button = ctk.CTkButton(self.control_frame, text="3. Gerar Áudio (TTS)", command=self.start_tts_task, state="disabled")
    ...
    def start_task(self, target_function):
        ...
        if not url:
            ...
            return
        self.log(f"--- Iniciando Tarefa: {target_function.__name__} ---")
        threading.Thread(target=self.run_task, args=(target_function, url)).start()

    def start_translate_task(self):
        if not self.last_original_file_path:
            self.log("!! ERRO: Nenhuma transcrição original para traduzir.")
            return
        self.log("--- Iniciando Tarefa: Tradução ---")
        threading.Thread(target=self.run_task, args=(translate_file_local, self.last_original_file_path)).start()

    def start_tts_task(self):
        if not self.last_translated_file_path:
            self.log("!! ERRO: Nenhum arquivo traduzido para gerar áudio.")
            return
        self.log("--- Iniciando Tarefa: Geração de Áudio (TTS) ---")
        threading.Thread(target=self.run_task, args=(generate_tts_audio, self.last_translated_file_path)).start()

    def run_task(self, target_function, *args):
        self.update_buttons_state("disabled")
        success, message, result_path = target_function(*args, log_callback=self.log)
        if success:
            self.log(f"++ SUCESSO: {message}")
            if target_function in [download_transcript, transcribe_audio_local]:
                self.last_original_file_path = result_path
            elif target_function == translate_file_local:
                self.last_translated_file_path = result_path
        else:
    ...
    def reset_state(self):
        ...
        self.log_textbox.configure(state="disabled")
        self.last_original_file_path, self.last_translated_file_path = None, None
        self.update_buttons_state("normal")
    ...
    def update_buttons_state(self, state):
        ...
        self.translate_button.configure(state="disabled" if is_running else "normal" if self.last_original_file_path else "disabled")
        self.tts_button.configure(state="disabled" if is_running else "normal" if self.last_translated_file_path else "disabled")
